...
### HIGH — Type Hints (target Python 3.11+)
- Public functions without type annotations
- Using `Any` when specific types are possible
- Missing `| None` for nullable parameters
- Use built-in generics: `list[X]`, `dict[K, V]`, `tuple[X, ...]` — flag any new `typing.List/Dict/Tuple/Optional/Union` as outdated
- Prefer `X | Y` over `Union[X, Y]`, `X | None` over `Optional[X]`
- Use `Self` (from `typing`) for fluent returns; `TaskGroup` over manual `gather` for structured concurrency
